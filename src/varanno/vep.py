...
def vep_api_hgvs_get(hgvs_string: HGVSString, species: str = "human") -> list[dict]:
...
def batch_vep_hgvs(hgvs_strings: list[HGVSString], species: str = "human") -> list[dict]:
...
def find_vep_allele_string(data: dict) -> float:
    """Retrieves "allele_string" from the VEP API data."""
    return data.get("allele_string")


def find_vep_variant_effect(data: dict) -> float:
